...
describe("Agent CRUD integration tests", () => {
  ...
  it("POST /v1/agents should create an agent", async () => {
    ...
    const res = await request(app).post(`/v1/agents?api-key=${apiKey}`).send(payload);
  ...
  it("PUT /v1/agents/:id should update the agent", async () => {
    if (!createdId) return;
    const res = await request(app).put(`/v1/agents/${createdId}?api-key=${apiKey}`).send({ name: "updated-name" });
    expect(res.status).toBe(200);
